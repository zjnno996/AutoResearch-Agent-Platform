...
def test_quality_and_experiment_gates_read_current_stage_layout(run_dir: Path) -> None:
    quality_dir = run_dir / "stage-23"
    quality_dir.mkdir()
    (quality_dir / "quality_report.json").write_text(
        '{"score_1_to_10": 8.4}',
        encoding="utf-8",
    )
    assert rc_runner._read_quality_score(run_dir) == 8.4

    analysis_dir = run_dir / "stage-16"
    analysis_dir.mkdir()
    (analysis_dir / "experiment_summary.json").write_text(
        '{"metrics_summary": {"accuracy": 0.81, "latency": 12.3}}',
        encoding="utf-8",
    )
    assert rc_runner._check_experiment_quality(run_dir, 0) == (
        True,
...
def test_partial_finalization_summary_reports_cumulative_progress(run_dir: Path) -> None:
    (run_dir / "checkpoint.json").write_text(
        '{"last_completed_stage": 26, "last_completed_name": "CITATION_VERIFY"}',
        encoding="utf-8",
    )
    summary = rc_runner._build_pipeline_summary(
        run_id="run-resume-s26",
        results=[_done(Stage.CITATION_VERIFY)],
        from_stage=Stage.CITATION_VERIFY,
        run_dir=run_dir,
    )

    assert summary["stages_total"] == 26
    assert summary["stages_executed"] == 26
    assert summary["stages_done"] == 26
    assert summary["invocation_stages_executed"] == 1
...
def test_proceed_decision_does_not_trigger_rollback(
    ...
    # Should execute the complete stage sequence, with no rollback.
...
def test_package_deliverables_includes_user_facing_evidence_boundaries(
    run_dir: Path, rc_config: RCConfig
) -> None:
    _setup_stage_artifacts(run_dir)
    ...
    (s9 / "exp_plan_diagnostics.json").write_text(
        json.dumps({"degraded": True, "user_facing_status_zh": "实验计划降级"}),
        encoding="utf-8",
    )

    s16 = run_dir / "stage-16"
    s16.mkdir(parents=True, exist_ok=True)
    (s16 / "experiment_provenance.json").write_text(
        json.dumps({
            "executed": True,
    ...
    (s16 / "experiment_summary.json").write_text(
        json.dumps({"total_runs": 1}),
        encoding="utf-8",
    )
    (s16 / "analysis.md").write_text(
        "# Analysis\n\nSmoke only.",
        encoding="utf-8",
    )
    s17 = run_dir / "stage-17"
    s17.mkdir(parents=True, exist_ok=True)
    (s17 / "research_readiness.json").write_text(
        json.dumps({"readiness_level": "engineering_smoke_only", "writing_policy": "engineering_report_only"}),
        encoding="utf-8",
    )
    (s17 / "decision_structured.json").write_text(
        json.dumps({"decision": "proceed", "writing_policy": "engineering_report_only"}),
        encoding="utf-8",
    )

    s25 = run_dir / "stage-25"
    s25.mkdir(parents=True, exist_ok=True)
    (s25 / "compilation_quality.json").write_text(
        json.dumps({"page_count": 3, "warnings": []}),
        encoding="utf-8",
    )

    dest = rc_runner._package_deliverables(run_dir, "run-boundaries", rc_config)
    ...
    manifest = json.loads((dest / "manifest.json").read_text())
    assert manifest["paper_pdf_available"] in (True, False)
    assert "user_notice_zh" in manifest
    assert "experiment_provenance.json" in manifest["notes"]
    assert "research_readiness.json" in manifest["notes"]
...
def test_forced_writing_transition_preserves_scientific_refine_decision(run_dir: Path) -> None:
    stage = run_dir / "stage-17"
    stage.mkdir(parents=True, exist_ok=True)
    (stage / "research_readiness.json").write_text(
        json.dumps({
            "decision": "refine",
            "writing_policy": "limited_claims_only",
            "user_facing_status_zh": "证据有限。",
        }),
        encoding="utf-8",
    )
    (stage / "decision_structured.json").write_text(
        json.dumps({"decision": "refine", "writing_policy": "limited_claims_only"}),
        encoding="utf-8",
    )

    rc_runner._mark_forced_writing_transition(run_dir, "maximum pivots reached")

    readiness = json.loads((stage / "research_readiness.json").read_text())
    decision = json.loads((stage / "decision_structured.json").read_text())
    assert readiness["decision"] == "refine"
    assert readiness["execution_control_decision"] == "proceed_to_writing"
    assert readiness["forced_proceed_after_max_pivots"] is True
