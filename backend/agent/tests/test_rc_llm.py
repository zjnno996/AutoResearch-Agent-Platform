...
def test_gpt55_pro_uses_responses_api_with_high_reasoning(
    monkeypatch: pytest.MonkeyPatch,
):
    captured: dict[str, object] = {}
    response = {
    ...
    def fake_urlopen(req: urllib.request.Request, timeout: int) -> _DummyHTTPResponse:
        captured["request"] = req
        captured["timeout"] = timeout
        return _DummyHTTPResponse(response)
    ...
    parsed = client.chat([{"role": "user", "content": "ping"}], max_tokens=8)

    request = captured["request"]
    assert isinstance(request, urllib.request.Request)
    assert request.full_url == "https://yunwu.ai/v1/responses"
    data = request.data
    assert isinstance(data, bytes)
    body = json.loads(data.decode("utf-8"))
...
def test_responses_api_incomplete_without_text_retries(monkeypatch: pytest.MonkeyPatch):
    calls = {"count": 0}
    responses = [
        {
            "model": "gpt-5.5-pro",
    ...
    def fake_urlopen(req: urllib.request.Request, timeout: int) -> _DummyHTTPResponse:
        ...
        idx = calls["count"]
        calls["count"] += 1
        return _DummyHTTPResponse(responses[idx])
