...
def test_ccf_annotation_preserves_frozen_paper_model() -> None:
    original = _make_paper(venue="NeurIPS")

    annotated = annotate_ccf_tier([original])

    assert annotated[0] is not original
    assert annotated[0]._ccf_tier == "CCF-A"
    assert annotated[0]._ccf_weight == 3.0
