...
_INERTIAL_SENSING_KEYWORDS = (
    "imu", "inertial", "accelerometer", "gyroscope", "wearable sensor",
    "activity recognition", "惯性", "加速度计", "陀螺仪", "传感器感知",
)
...
class SurveyorAgent(BaseAgent):
    ...
    def execute(self, context: dict[str, Any]) -> AgentStepResult:
        ...
        combined_topic = f"{topic} {hypothesis}".lower()
        # Sensor topics are often wrapped in a generic CV role prompt containing
        # words such as self-supervised or diffusion.  Those broad terms used to
        # route IMU projects to CIFAR/MAML.  A direct modality match must win.
        if (
            "inertial_sensing" in self._knowledge
            and any(keyword in combined_topic for keyword in _INERTIAL_SENSING_KEYWORDS)
        ):
            domain_ids = ["inertial_sensing"]
