"""Conference template definitions for NeurIPS, ICLR, ICML, and IEEE.
...
class ConferenceTemplate:
    ...
    def render_preamble(
        ...
        keyword_block = ""
        if self.author_format == "ieee" and keywords.strip():
            keyword_block = (
                "\n\\begin{IEEEkeywords}\n"
                f"{keywords}\n"
                "\\end{IEEEkeywords}\n"
        ...
        class_options = f"[{self.document_class_options}]" if self.document_class_options else ""
        return (
            f"{style_comment}"
            f"\\documentclass{class_options}{{{self.document_class}}}\n"
            f"{style_line}"
            f"{pkg_lines}\n"
            f"{preamble_extra}\n"
            f"\n"
...
IEEE = ConferenceTemplate(
    name="ieee",
    display_name="IEEE Conference Paper",
    year=2026,
    ...
    bib_style="IEEEtran",
    columns=2,
    style_download_url="https://www.ieee.org/conferences/publishing/templates.html",
    preamble_extra="\\IEEEoverridecommandlockouts",
    document_class_options="conference",
)
