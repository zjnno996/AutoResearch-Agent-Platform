...
def markdown_to_latex(
    ...
    *,
    ...
    preamble = template.render_preamble(
        ...
        keywords=_convert_inline(keywords),
    )
...
def _extract_keywords(sections: list[_Section]) -> str:
    """Extract IEEE index terms/keywords when the manuscript provides them."""
    for sec in sections:
        if sec.heading_lower in {"keywords", "index terms", "index-terms"}:
            return sec.body.strip().replace("\n", " ")
    return ""
...
_SKIP_HEADINGS = {"title", "abstract", "keywords", "index terms", "index-terms"}
...
def _render_table(table_lines: list[str], caption: str = "") -> str:
    ...
    if needs_resize:
        # IEEE conference output is two-column; \textwidth spans both columns
        # and makes an ordinary table overrun its float box.  \columnwidth is
        # correct for the single-column table environment emitted here.
    ...
    for sec in sections:
        if sec.level in (1, 2) and sec.heading:
            ...
            if heading_lower in _EXPECTED_SECTIONS:
                ...
                # A combined heading such as "Experiments and Results" must
                # satisfy both requirements; stopping after the first substring
                # produced a false "Missing sections: result" warning.
